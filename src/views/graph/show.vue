<template>
  <div>
    ...
    <div
      style="width: 95%;
        margin-left: 30px;
        margin-top: 10px;padding:20px;border-radius: 4px;border:2px;background: #d2e9ff;box-shadow: 0 2px 4px rgba(0, 0, 0, .12), 0 0 6px rgba(0, 0, 0, .04)"
    >
...
<!--      </el-select>-->
    ...
    >
      <div slot="title" class="dialog-title">
        ...
        <div class="button-right">
          <el-button type="primary" @click="detailsDialogVisible=false" style="float: right;"
          >关 闭
          </el-button>
        </div>
      </div>
      <equipmentTotally :editValue="nodeDetails"/>
    </el-dialog>
    ...
    >
      <div slot="title" class="dialog-title">
        <!--        <span class="title-text" style="font-size: 20px;">设备详情</span>-->
        <div class="button-right">
          <el-button type="primary" @click="fixDialogVisible=false" style="float: right;"
          >关 闭
          </el-button>
        </div>
      </div>
      <nodeFixPlan :devObject="nodeDetails" />
    ...
    >
      <div slot="title" class="dialog-title">
        <!--        <span class="title-text" style="font-size: 20px;">设备详情</span>-->
        <div class="button-right">
          <el-button type="primary" @click="faultDialogVisible=false" style="float: right;"
          >关 闭
          </el-button>
        </div>
      </div>
      <nodeFault :devObject="nodeDetails"/>
...
import EquipmentTotally from '@/views/life/equipmentTotally.vue'
import NodeFixPlan from '../graph/nodeFixPlan.vue'
import NodeFault from '../graph/nodeFault.vue'
...
export default {
  ...
  components: {
    EquipmentTime, EquipmentTotally, NodeFixPlan, NodeFault
  },
  methods: {
    ...
    //通过路由接受到响应图谱的Id 展示相应的图谱
    // getGraphIdAndShowGraph() {
    //   this.graphOptionValue = this.$route.query.graphId
    //   this.showGraph()
    // },
    ...
    getGraphDataAndCreate() {
      // listAllTree({}).then((response) => {
      //如果路由传递过来的GraphId 不是未定义
      // if(typeof(this.$route.query.graphId)=='undefined'){
    ...
    createGraph: function() {
      ...
      graph.on('image-shape0:click', (evt) => {
        ...
        // alert('设备详情')
        // if(orgId==1||orgId==-1)
        if(this.nodeDetails.devAttrCode=='11'||this.nodeDetails.devAttrCode=='10'){
          alert("该节点不是设备节点,不能查看详情")
          return
        }
  ...
  mounted() {
    if (typeof (this.$route.query.graphId) == 'undefined') {
      ...
      getGraph(graphId).then(response=>{
        ...
        this.graphOptionValue = graph.graphName
        this.graphData.nodes = JSON.parse(graph.graphNodes)
        this.graphData.edges = JSON.parse(graph.graphEdges)
        this.createGraph()
      })
    }
    this.getGraphList()

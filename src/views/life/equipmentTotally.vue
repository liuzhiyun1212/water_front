<template>
      <table class="table" style="margin-left:10%;width:80%;height:100%;border:1px solid;align:center;font-size:10px;"
...
export default {
  ...
  methods: {
    getParams() {
      ...
      if(this.$route.query.devCode != undefined){
        ...
        if(a.useStatus == 0){
          this.ruleForm.status = "运行中"
        }else if(a.useStatus == -1){
          this.ruleForm.status = "未运行"
        }else if(a.useStatus == -2){
          this.ruleForm.status = "维保中"
        }else if(a.useStatus == -3){
          this.ruleForm.status = "已报废"
        }
        ...
        if (a.equipmentPicture != null) {
          this.ruleForm.equipmentPicture = a.equipmentPicture;
        }
        if(a.file != null){
          this.ruleForm.file = a.file;
        }
        if(a.runningTime != null){
          this.ruleForm.runningTime = a.runningTime;
        }
      }

    ...
    <el-dialog :title="title" :visible.sync="open" width="600px" append-to-body>
      <el-form ref="form" :model="form" :rules="rules" label-width="80px">
        <el-row>
          ...
          <el-col :span="12">
            <el-form-item label="归属部门" prop="deptId">
              <treeselect v-model="form.deptId" :options="deptOptions1" :show-count="true" placeholder="请选择归属部门" />
            </el-form-item>
          </el-col>
        </el-row>
        <el-row>
        ...
        <el-row>
          ...
          <el-col :span="12">
            <el-form-item v-if="false" label="用户密码" prop="password">
...
export default {
  ...
  methods: {
    ...
    getTreeselect() {
      this.deptOptions = []
      var all = {id:null,label:"全部"}
      treeselect().then(response => {
        this.deptOptions.push(all);
        for(let i = 0;i<response.data.length;i++){
          this.deptOptions.push(response.data[i]);
        }
      });
    },
    getTreeselect1() {
    ...
    handleAdd() {
      ...
      getUser().then(response => {
        ...
        this.open = true;
        this.title = "添加用户";
        this.form.password = this.initPassword2;
      });
    },
    /** 修改按钮操作 */
    handleUpdate(row) {
    ...
    /** 重置密码按钮操作 */
    handleResetPwd(row) {
      resetUserPwd(row.userId, 123).then(response => {
            this.$modal.msgSuccess("修改成功，新密码是：" + 123);
      });
    },
    /** 分配角色操作 */
    ...
    /** 提交按钮 */
    submitForm: function() {
      this.form.roleName = this.rolemake(this.roleOptions,this.form.roleIds);

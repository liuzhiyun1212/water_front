    ...
    <el-card class="box-card">
      <el-row :gutter="15">
        ...
        >
          ...
          <el-col :span="6">
            <el-form-item label="设备编号" prop="field105">
              <el-select
      ...
      <el-row>
        <div id="graphBox" :style="{ height: graphHeight }"></div>
      </el-row>
    </el-card>
  </div>
</template>
...
import { initGraphApi } from "@/api/system/riskGraph";
export default {
  components: {},
  props: [],
  data() {
    return {
      graph: null,
      preNode: {}, //点击事件 保存的上一个节点  当点击下一个节点 上一个节点 功能按钮收缩
      graphData: {
        // 点集
        nodes: [],
        // 边集
  ...
  methods: {
    init() {
      this.graphHeight = document.body.clientHeight * 0.7 + "px";
    },
    getGragpData(root) {
      console.log(root);
    ...
    initgraph() {
      initGraphApi().then((res) => {
        console.log(res);
        this.getGragpData(res.root);
        ...
        for (var item in res.nodeMap) {
          if (res.nodeMap[item].type == "a" || res.nodeMap[item].type == "b") {
            this.graphData.nodes.push({
              id: res.nodeMap[item].id,
              label: res.nodeMap[item].name,
              data: res.nodeMap[item].name,
              size: 100,
              type: "customNode4",
              style: {
                stroke: "#fff",
        ...
        graph.on("node:click", (event) => {
          ...
          item._cfg.group.cfg.children.forEach((child, i) => {
            if (
              child.cfg.name === "key-shape" &&
              child.cfg.name !== "text-shape"
            ) {
                ...
                (ratio) => {
                  ...
                  return {
                    ...
                    x:
                      -20 +
                      centerCircleRadius *
                        Math.cos((Math.PI * 2 * i) / circleList.length - 1) -
                      40 *
                        Math.cos(
                          ratio * Math.PI +
                            (i / circleList.length - 1) * Math.PI * 2
                        ),
                    // y: 40 * Math.sin(Math.PI * 2 * i / circleList.length - 1),
                    y:
                      -20 +
                      centerCircleRadius *
                        Math.sin((Math.PI * 2 * i) / circleList.length - 1) -
                      40 *
                        Math.sin(
                          ratio * Math.PI +
                            (i / circleList.length - 1) * Math.PI * 2
          ...
          if (arr.length > 1) {
            ...
            if (
              item._cfg.id !== that.preNode._cfg.id &&
              !that.preNode._cfg.group.cfg.destroyed
            ) {
    ...
    submitForm() {
      this.$refs["searchGraph"].validate((valid) => {
        if (!valid) return;
        // TODO 提交表单
      });
    },
    resetForm() {
      this.$refs["searchGraph"].resetFields();
    },
  },
};
</script>

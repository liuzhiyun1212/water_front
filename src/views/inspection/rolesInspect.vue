...
<script>
import { listInspectiononly, listInspection, getInspection, delInspection, addInspection, updateInspection } from "../../api/system/inspection";
  ...
  methods: {
    ...
    getList() {
      ...
      listInspectiononly(this.queryParams).then(response => {
        ...
        /*for(let i=0;i<inspectionList1.length;i++){
          if(inspectionList1[i].inspectPeople===this.$store.state.user.name){
            this.inspectionList.push(inspectionList1[i]);
            count++;
          }}*/
          /*this.inspectionList = inspectionList1;*/
    ...
    skipto: function (value) {
      ...
      console.log(value);
      if(value.value.maintenanceName===null){
      this.$router.push({
        path: "graph/fix/create",
        query: {
         inspection: value,
        },
      });} else if(value.value===''){this.$modal.msgError("请先选择一条巡检记录！");}
      else{this.$modal.msgError("该记录无法再次生成维保计划！");}
    },
